...
async function startGenerate(){
  ...
  try{
    var r=await authFetch(API+'/analyze',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({youtube_url:url,url:url})});
    var d=await r.json();
    if(!r.ok||d.error)throw new Error(d.error||'Failed to start');
    currentProjectId=d.project_id||d.id||d.job_id||d.jobId;
...
async function pollStatus(){
  ...
  try{
    var r=await authFetch(API+'/analyze/'+projectId);
    var d=await r.json();
...
async function loadClips(projectId){
  try{
    var r=await authFetch(API+'/clips?job_id='+projectId+'&project_id='+projectId);
    var d=await r.json();
    var clips=d.clips||d||[];
    if(!Array.isArray(clips)||clips.length===0){showToast('No clips found.');return}
    renderClips(clips);
...
function renderClips(clips){
  ...
  clips.forEach(function(clip,i){
    var score=clip.viral_score||clip.score||Math.floor(75+Math.random()*22);
    var title=clip.title||'Viral Clip #'+(i+1);
    var hook=clip.hook||clip.hook_line||'';
    var tags=clip.hashtags||(clip.tags?clip.tags.join(' '):'');
    var dur=clip.duration?Math.round(clip.duration)+'s':'~12s';
    var url=clip.url||clip.download_url||clip.video_url||'';
